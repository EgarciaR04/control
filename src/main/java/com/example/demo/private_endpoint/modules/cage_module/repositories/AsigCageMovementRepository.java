...
public interface AsigCageMovementRepository extends JpaRepository<AsigCageMovement, Long> {
    @Query("SELECT asCM FROM AsigCageMovement asCM WHERE asCM.animal_movement.user.company.id = :id_company")
    List<AsigCageMovement> findReportCageMovementByCompanyId(@Param("id_company") long id_company);
